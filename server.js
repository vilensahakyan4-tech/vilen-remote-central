...
function writeStream(client,event){
  ...
  if(isFrame&&client.backpressured){client.latestFrame=event;droppedFrames++;return;}
  const writable=client.res.write(`data: ${JSON.stringify(event)}\n\n`);
  if(isFrame&&!writable){
    client.backpressured=true;
    client.res.once('drain',()=>{client.backpressured=false;const latest=client.latestFrame;client.latestFrame=null;if(latest)writeStream(client,latest);});
  }
}
...
http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&url.pathname==='/health')return send(res,200,{ok:true,service:'Vilen Remote Central',online:devices.size,droppedFrames});
  if(req.method==='GET'&&url.pathname==='/events'){
    ...
    const client={res,backpressured:false,latestFrame:null};if(!streams.has(key))streams.set(key,new Set());streams.get(key).add(client);flush(key,client);req.on('close',()=>{const set=streams.get(key);set?.delete(client);if(!set?.size)streams.delete(key);});return;
  }
